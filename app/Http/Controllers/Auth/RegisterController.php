...
//        return $user;
    ...
    public function register(Request $request) {
        event(new RegisteredEvent(RegisteredEvent::JOB_TYPE_1, $request->all()));
    ...
    public function showForm($email_token) {
        ...
        if (!User::where('email_verify_token', $email_token)->exists()) {
            ...
            // 本登録済みユーザーか
            if ($user->email_verified == config('const.USER_STATUS.REGISTER')) { //REGISTER=1
    ...
    public function mainRegister(Request $request) {
        ...
        $user->email_verified = config('const.USER_STATUS.REGISTER');
        $user->name = $request->name;
        $user->name_pronunciation = $request->name_pronunciation;
        $user->birth_year = $request->birth_year;
        $user->birth_month = $request->birth_month;
